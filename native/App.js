...
function AIQuizLauncher({type,setType}){
 const [subject,setSubject]=useState('العربية'),[paperType,setPaperType]=useState('بابل شيت فقط (CQ)'),[scope,setScope]=useState('درس محدد 🎯'),[topic,setTopic]=useState('النحو'),[count,setCount]=useState(4),[level,setLevel]=useState('كافة المستويات'),[busy,setBusy]=useState(false),[qs,setQs]=useState(null),[idx,setIdx]=useState(0),[score,setScore]=useState(0),[answer,setAnswer]=useState(null),[essay,setEssay]=useState(''),[feedback,setFeedback]=useState(''),[aiSessionId,setAiSessionId]=useState(null);
 const topics=TOPICS[subject]||['الدرس الحالي'];
 async function gen(){
  const {data}=await supabase.auth.getSession();if(!data.session)return Alert.alert('تسجيل الدخول مطلوب');setBusy(true);
  try{
   ...
   const d=await r.json();if(!r.ok||d.error)throw Error(d.error||'تعذر توليد الأسئلة');if(!Array.isArray(d.questions)||!d.questions.length)throw Error('لم يُرجع محرك الذكاء الاصطناعي أسئلة.');
   const sess=await supabase.from('ai_exam_sessions').insert({user_id:data.session.user.id,subject,topic,scope:scopeType,exam_type:qtype,level:level,questions:d.questions,answers:{},total_questions:d.questions.length,status:'active'}).select('id').single();
   if(sess.error)console.log('ai session save',sess.error.message);setAiSessionId(sess.data?.id||null);
 ...
 async function persistAnswer(q,selectedIndex,correct){
  const {data}=await supabase.auth.getSession();if(!data.session)return;
  if(aiSessionId)await supabase.from('ai_exam_sessions').update({answers:{[String(idx)]:selectedIndex}}).eq('id',aiSessionId).eq('user_id',data.session.user.id);
  if(!correct)await supabase.from('student_mistakes').insert({user_id:data.session.user.id,subject,topic,question:q.question||q.stem,options:q.options||[],correct_index:Number(q.correct??q.correctIndex??0),student_answer:String(selectedIndex),explanation:q.explanation||'',source:'ai_exam'}).catch(()=>{});
 }
 async function essayGrade(q){
  const {data}=await supabase.auth.getSession();if(!data.session)return;
  try{const r=await fetch(AI_URL,{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+data.session.access_token,'apikey':SUPABASE_KEY},body:JSON.stringify({action:'grade',question:q.question||q.stem,answer:essay,rubric:q.rubric||'',modelAnswer:q.modelAnswer||''})});const d=await r.json();setFeedback('الدرجة: '+(d.score??0)+' — '+(d.feedback||''));if(aiSessionId)await supabase.from('ai_exam_sessions').update({answers:{[String(idx)]:essay}}).eq('id',aiSessionId).eq('user_id',data.session.user.id)}catch(e){setFeedback(e.message)}
 }
 async function finish(finalScore){
  const {data}=await supabase.auth.getSession();if(!data.session)return;
  if(aiSessionId)await supabase.from('ai_exam_sessions').update({score:finalScore,total_questions:qs?.length||0,status:'completed',updated_at:new Date().toISOString()}).eq('id',aiSessionId).eq('user_id',data.session.user.id);
  await supabase.from('exam_results').insert({user_id:data.session.user.id,subject,exam_type:type,scope:scope,score:finalScore,total_questions:qs?.length||0,details:{topic,level}}).catch(()=>{});
 }
 if(qs){
  if(idx>=qs.length){finish(score);return <View><Text style={s.h2}>انتهى الاختبار 🎉</Text><Text style={s.big}>{score}/{qs.length}</Text><Btn secondary onPress={()=>{setQs(null);setAiSessionId(null)}}>اختبار جديد</Btn></View>}
  const q=qs[idx],isEssay=q.type==='essay'||type==='essay';
  return <View><View style={s.row}><Text style={s.pill}>{idx+1} / {qs.length}</Text><Text style={s.pill}>{level}</Text></View><Text style={s.quizQ}>{q.question||q.stem}</Text>{isEssay?<><Field value={essay} onChangeText={setEssay} placeholder="اكتب إجابتك..." multiline/><Btn onPress={async()=>{await essayGrade(q);setIdx(i=>i+1);setEssay('')}}>تصحيح الإجابة</Btn></>:(q.options||[]).map((o,i)=><Pressable key={i} disabled={answer!==null} onPress={async()=>{setAnswer(i);const ok=Number(q.correct)===i||Number(q.correctIndex)===i;await persistAnswer(q,i,ok);if(ok)setScore(x=>x+1);setTimeout(()=>{setAnswer(null);setIdx(x=>x+1)},450)}} style={[s.answer,answer===i&&(Number(q.correct)===i||Number(q.correctIndex)===i?s.correct:s.wrong)]}><Text>{o}</Text></Pressable>)}{feedback?<Card><Text style={s.muted}>{feedback}</Text></Card>:null}</View>
 }
 return <View><View style={s.tabs}><Pressable onPress={()=>setType('mcq')} style={[s.tab,type==='mcq'&&s.tabActive]}><Text>اختياري</Text></Pressable><Pressable onPress={()=>setType('essay')} style={[s.tab,type==='essay'&&s.tabActive]}><Text>مقالي</Text></Pressable><Pressable onPress={()=>setType('mixed')} style={[s.tab,type==='mixed'&&s.tabActive]}><Text>مختلط</Text></Pressable></View><Choice label="المادة" value={subject} options={Object.keys(TOPICS)} onChange={v=>{setSubject(v);setTopic((TOPICS[v]||['الدرس الحالي'])[0])}}/><Choice label="نوع الورقة" value={paperType} options={['بابل شيت فقط (CQ)','مقالي','مختلط']} onChange={setPaperType}/><Choice label="نطاق الامتحان" value={scope} options={['درس محدد 🎯','فصل','المادة بالكامل']} onChange={setScope}/><Choice label="الدرس / الفصل" value={topic} options={topics} onChange={setTopic}/><View style={s.aiLocked}><Text style={s.aiLockedText}>🔒 القسم الحالي: {topic} — لن يتم خلطه بأي قسم آخر</Text></View><View style={s.row}><View style={s.half}><Choice label="المستوى" value={level} options={['كافة المستويات','سهل','متوسط','صعب']} onChange={setLevel}/></View><View style={s.half}><Choice label="عدد الأسئلة" value={String(count)+' أسئلة'} options={['4 أسئلة','6 أسئلة','10 أسئلة','20 أسئلة']} onChange={v=>setCount(Number(String(v).replace(/\\D/g,''))||4)}/></View></View><Btn disabled={busy} onPress={gen}>🚀 توليد وبدء الامتحان بالذكاء الاصطناعي ✨</Btn></View>
}
...
function Gradebook({state,patch,session,notify}){const [subject,setSubject]=useState(state.selected[0]||'العربية'),[score,setScore]=useState(''),[exam,setExam]=useState('اختبار'),[rows,setRows]=useState([]);useEffect(()=>{if(!session)return;(async()=>{const r=await supabase.from('exam_results').select('*').eq('user_id',session.user.id).order('created_at',{ascending:false}).limit(50);if(!r.error)setRows(r.data||[])})()},[session]);async function add(){if(!score)return;const r=await supabase.from('exam_results').insert({user_id:session.user.id,subject,exam_type:exam,scope:'manual',score:Number(score),total_questions:100,details:{source:'manual'}}).select().single();if(r.error)return notify('تعذر حفظ الدرجة: '+r.error.message);setRows(x=>[r.data,...x]);setScore('');notify('تم حفظ الدرجة ✅')}return <ScrollView style={s.scroll}><Title title="سجل الدرجات 📊" sub="درجاتك محفوظة على حسابك"/><View style={s.pad}><Card><Field value={subject} onChangeText={setSubject} placeholder="المادة"/><Field value={exam} onChangeText={setExam} placeholder="اسم الاختبار"/><Field value={score} onChangeText={setScore} keyboardType="numeric" placeholder="الدرجة من 100"/><Btn onPress={add}>حفظ الدرجة</Btn></Card><Card>{rows.length?rows.map((x,i)=><View key={x.id||i} style={s.list}><Text>{x.subject} — {x.exam_type}</Text><Text style={s.bigMini}>{x.score}/100</Text></View>):<Text style={s.muted}>أضف أول درجة لتظهر هنا.</Text>}</Card></View></ScrollView>}
function GoldenNotes({state,patch,notify,session}){const [title,setTitle]=useState(''),[body,setBody]=useState(''),[rows,setRows]=useState(state.goldenNotes||[]);useEffect(()=>{if(!session)return;(async()=>{const r=await supabase.from('golden_notes').select('*').eq('user_id',session.user.id).order('created_at',{ascending:false}).limit(50);if(!r.error)setRows(r.data||[])})()},[session]);async function add(){if(!title.trim()||!body.trim())return notify('اكتب عنوان الفكرة وشرحها');const r=await supabase.from('golden_notes').insert({user_id:session.user.id,title:title.trim(),description:body.trim()}).select().single();if(r.error)return notify('تعذر حفظ الفكرة: '+r.error.message);setRows(x=>[r.data,...x]);patch({goldenNotes:[...(state.goldenNotes||[]),r.data]});setTitle('');setBody('');notify('تم حفظ الفكرة الذهبية ✅')}return <ScrollView style={s.scroll}><Title title="الأفكار الذهبية 💡" sub="حفظ دائم على حسابك"/><View style={s.pad}><Card><Field value={title} onChangeText={setTitle} placeholder="عنوان الفكرة"/><Field value={body} onChangeText={setBody} placeholder="اكتب الفكرة أو القاعدة..." multiline/><Btn onPress={add}>حفظ الفكرة الذهبية</Btn></Card>{rows.map((x,i)=><Card key={x.id||i}><Text style={s.h2}>💡 {x.title}</Text><Text style={s.muted}>{x.description||x.body}</Text></Card>)}</View></ScrollView>}
function Subscription({profile,session,notify}){const [ref,setRef]=useState(''),[plan,setPlan]=useState('باقة الشهر (50 جنيه)'),[phone,setPhone]=useState('');const plans={'باقة الشهر (50 جنيه)':50,'باقة 3 شهور (120 جنيه)':120,'باقة السنة (300 جنيه)':300};async function send(){if(!ref.trim())return notify('اكتب رقم العملية أولاً');const {data:u}=await supabase.auth.getUser();const r=await supabase.from('payment_submissions').insert({user_id:u.user.id,name:profile?.display_name||profile?.name||'',email:u.user.email||'',phone,transaction_id:ref.trim(),plan,amount:plans[plan]}).select().single();if(r.error)return notify('تعذر إرسال الطلب: '+r.error.message);setRef('');notify('تم إرسال طلب الدفع للمراجعة ✅')}return <ScrollView style={s.scroll}><Title title="الاشتراك 💎" sub="استمر في رحلتك التعليمية بدون توقف"/><View style={s.pad}><Card><Text style={s.big}>💎</Text><Text style={s.h2}>اشتراك رحلة الثانوية</Text><Text style={s.muted}>اختر الباقة ثم أرسل رقم العملية بعد الدفع.</Text><Choice label="الباقة" value={plan} options={Object.keys(plans)} onChange={setPlan}/><Field value={phone} onChangeText={setPhone} placeholder="رقم الهاتف"/><Field value={ref} onChangeText={setRef} placeholder="رقم العملية / المرجع"/><Btn onPress={send}>💳 إرسال طلب الاشتراك</Btn></Card><Card><Text style={s.h2}>حالة الحساب</Text><Text style={s.muted}>{profile?.is_paid?'💎 الاشتراك مفعل':'⏳ لم يتم تأكيد الاشتراك بعد'}</Text></Card></View></ScrollView>}
