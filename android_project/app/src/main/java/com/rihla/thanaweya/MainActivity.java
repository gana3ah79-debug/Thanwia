...
public class MainActivity extends Activity{
 WebView web;
 private static final String INJECT="javascript:(function(){try{var a=[['rihlaAuthFixV2','auth-fix-v2.js'],['rihlaAppStabilityV4','app-stability-v4.js'],['rihlaNavFixV5','app-nav-fix-v5.js'],['rihlaNavigationLogoutV6','app-navigation-logout-v6.js'],['rihlaAIUX','aiux.js'],['rihlaFinalUI2027','ui-final-2027.js'],['rihlaSubscriptionV2','subscription-v2.js'],['rihlaAdminAccess','admin-access-v2.js']];a.forEach(function(x){if(!document.getElementById(x[0])){var s=document.createElement('script');s.id=x[0];s.src='file:///android_asset/www/'+x[1];document.body.appendChild(s)}})}catch(e){console.log('inject',e)}})();";
 ...
 @Override public void onCreate(Bundle b){super.onCreate(b);web=new WebView(this);web.setLayoutParams(new ViewGroup.LayoutParams(-1,-1));WebSettings s=web.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);s.setDatabaseEnabled(true);s.setAllowFileAccess(true);s.setAllowContentAccess(true);web.addJavascriptInterface(new AppBridge(this),"Android");web.setWebViewClient(new WebViewClient(){@Override public void onPageFinished(WebView v,String u){super.onPageFinished(v,u);v.evaluateJavascript(INJECT,null);}});setContentView(web);web.loadUrl("file:///android_asset/www/index.html");}
 @Override public void onBackPressed(){web.evaluateJavascript(BACK,null);}
 public static class AppBridge{private final Activity a;AppBridge(Activity a){this.a=a;}@JavascriptInterface public void backResult(String r){if("exit".equals(r))a.finishAndRemoveTask();}@JavascriptInterface public void exitApp(){a.runOnUiThread(()->a.finishAndRemoveTask());}}
}
