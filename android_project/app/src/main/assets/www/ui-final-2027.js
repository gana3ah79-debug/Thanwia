...
function removeOldExit(){
 document.querySelectorAll('#rihlaExitApp,#rihlaExitV3,.rihla-exit-button,[data-exit-app="true"]').forEach(function(e){e.remove();});
 document.querySelectorAll('button,a').forEach(function(e){var t=textOf(e);if(/^خروج(?:\s|$)/.test(t)||t.indexOf('خروج 🚪')===0||t.indexOf('خروج')===0&&t.length<35){if(!isStartHelp(e))e.remove();}});
}
function addExit(){
 removeOldExit();
 var target=null;
...
var queued=false;function run(){if(queued)return;queued=true;setTimeout(function(){queued=false;addExit();},80);}
